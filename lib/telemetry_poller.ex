defmodule Telemetry.Poller do
  ...
  By default, it polls all default VM measurements, which is equivalent to:
      ...
      # This is the default - you don't have to declare it
  ...
  pairs returned by `:erlang.memory/0` function, e.g. `:total` for total memory,
  ...
  * `:vm_measurements` - a list of atoms describing measurements related to the Erlang VM, or an
    atom `:default`, in which case default VM measurements are used. VM measurements are typically
    performed only by the built-in poller. For custom pollers, the default value is `[]`;
